...
def init_dataloader(cfg: DictConfig,
    ...
    if cfg.training.standard_scaler:
        train_idx = train_dataset.indices
        labels_train = train_dataset.dataset.tensors[2][train_idx]

        mean_train = labels_train.mean(dim=0)
        std_train = labels_train.std(dim=0)

        for i, task in enumerate(cfg.dataset.tasks):
            if not task.regression:
                mean_train[i] = 0
                std_train[i] = 1
        StandardScaler.mean = mean_train
        StandardScaler.std = std_train

        standard_scaler = StandardScaler()

        noramlized_labels_train = standard_scaler.transform(labels_train)

        train_dataset = utils.TensorDataset(train_dataset.dataset.tensors[0][train_idx], train_dataset.dataset.tensors[1][train_idx], noramlized_labels_train)

    sampler = None
    if "imbalanced" in cfg.dataset and cfg.dataset.imbalanced:
        unique_label, count = np.unique(temp_label, return_counts=True)

        count = 1.0 / count

        label_to_count = dict(zip(unique_label, count))

        samples_weight = np.array([label_to_count[t] for t in temp_label])
    ...
    train_dataloader = utils.DataLoader(
        train_dataset, batch_size=cfg.dataset.batch_size, shuffle=False if sampler is not None else True,
        drop_last=cfg.dataset.drop_last, sampler=sampler)
...
def init_stratified_dataloader(cfg: DictConfig,
    ...
    if "imbalanced" in cfg.dataset and cfg.dataset.imbalanced:
        ...
        samples_weight = np.array([label_to_count[t]
                                  for t in orignial_labels.tolist()])
