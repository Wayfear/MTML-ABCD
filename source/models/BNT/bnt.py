    ...
    def __init__(self, config: DictConfig):
        ...
        if self.pos_encoding == 'identity':
            self.node_identity = nn.Parameter(torch.zeros(
                config.dataset.node_feature_sz, config.model.pos_embed_dim), requires_grad=True)
            forward_dim = config.dataset.node_feature_sz + config.model.pos_embed_dim
        ...
        if config.model.preprocess:
            self.fc = nn.Sequential(
                nn.Linear(8 * sizes[-1], 256),
                nn.LeakyReLU(),
                nn.Linear(256, config.model.preprocess_dim),
            )
        else:
